...
export async function logLoginAttempt(success: boolean): Promise<void> {
  ...
  const insert = db.prepare(
    "INSERT INTO login_audit (ts, success) VALUES (?, ?)"
  );
...
export async function getInquiriesCount(): Promise<number> {
  const row = db.prepare("SELECT COUNT(*) as c FROM inquiries").get() as {
    c: number;
  };
  return row.c;
}

export async function getNewInquiriesCountSince(
  sinceTs: number
): Promise<number> {
  const row = db
...
export async function createInquiry(
  ...
  const insert = db.prepare(
    "INSERT INTO inquiries (name, email, institution, question, created_at) VALUES (?, ?, ?, ?, ?)"
  );
  ...
  try {
    const row = db
      .prepare(
        "SELECT id, name, email, institution, question, created_at FROM inquiries WHERE id = ?"
      )
...
export async function getNotificationEmail(): Promise<string | null> {
  const row = db
    ...
    .get(SETTINGS_EMAIL_KEY) as { value?: string } | undefined;
...
export type LandingSettings = {
  ...
  howItWorksImage?: string | null; // image for "How it works" section
  socials: { telegram: string; vk: string };
  socialsImage?: string | null; // image for "Social networks" section
  whyUseful: {
...
export async function getLandingSettings(): Promise<LandingSettings | null> {
  const row = db
    .prepare("SELECT value FROM settings WHERE key = ?")
    .get(SETTINGS_LANDING_KEY) as { value?: string } | undefined;
